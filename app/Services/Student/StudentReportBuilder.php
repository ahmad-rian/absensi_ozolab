...
class StudentReportBuilder
{
    ...
    public function attendanceXlsx(Request $request, Student $siswa): BinaryFileResponse
    {
        ...
        $data = $this->stats->attendanceFor($siswa, $start, $end, null);
    ...
    public function prayerXlsx(Request $request, Student $siswa): BinaryFileResponse
    {
        ...
        $data = $this->stats->prayerFor($siswa, $start, $end, $type, null);
    ...
    public function combinedPdf(Student $student, string $start, string $end, string $kind = 'semuanya'): HttpResponse
    {
        $sections = [];
        if (in_array($kind, ['semuanya', 'absensi'], true)) {
            $sections[] = ['title' => 'Absensi Sekolah', ...$this->stats->attendanceFor($student, $start, $end, null)];
        }
        foreach (['dhuha' => SchoolFeature::SholatDhuha, 'dzuhur' => SchoolFeature::SholatDzuhur] as $slug => $feature) {
            if (in_array($kind, ['semuanya', $slug], true) && $student->school && SchoolFeatures::for($student->school)->enabled($feature)) {
                $sections[] = ['title' => 'Sholat '.ucfirst($slug), ...$this->stats->prayerFor($student, $start, $end, PrayerType::fromSlug($slug), null)];
            }
        }

        return Pdf::loadView('pdf.student-combined', [
            'student' => $student->loadMissing(['classroom', 'school']), 'schoolName' => $student->school?->name ?? 'Sekolah',
            'startDate' => $start, 'endDate' => $end, 'sections' => $sections, 'printedAt' => SchoolTime::now()->format('d/m/Y H:i'),
        ])->setPaper('a4')->download($this->filename($kind, $student, $start, $end, 'pdf'));
    }
