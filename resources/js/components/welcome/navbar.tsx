...
export function Navbar() {
    ...
    useEffect(() => {
        if (mobileOpen) {
            ...
            const frame = requestAnimationFrame(() => setMobileVisible(false));
        ...
        <>
            ...
            {mobileOpen && (
                ...
                >
                    {/* Header */}
                    <div
                        className={`flex h-16 shrink-0 items-center justify-between px-4 transition-all duration-300 ${
                            mobileVisible
                    ...
                    <div className="flex flex-1 flex-col gap-0.5 overflow-y-auto px-3 pt-4">
                        {navLinks.map((link, i) => (
                            <a
                                key={link.href}
                                href={link.href}
                                className={`rounded-2xl px-5 py-4 text-[17px] font-medium text-foreground transition-all duration-300 ease-out hover:bg-accent/80 active:bg-accent ${
                                    mobileVisible
                                        ? 'translate-x-0 opacity-100'
                                        : 'translate-x-8 opacity-0'
                                }`}
                                style={{
                                    transitionDelay: mobileVisible
                                        ? `${60 + i * 40}ms`
                                        : '0ms',
                                }}
                                onClick={(e) => {
                    ...
                    {/* Bottom CTA */}
                    <div
                        className={`shrink-0 border-t border-border/50 p-4 pb-8 transition-all duration-300 ease-out ${
                            mobileVisible
                                ? 'translate-y-0 opacity-100'
                                : 'translate-y-6 opacity-0'
                        }`}
                        style={{
                            transitionDelay: mobileVisible ? '380ms' : '0ms',
                        }}
                    >
                        <Button
                            variant="outline"
